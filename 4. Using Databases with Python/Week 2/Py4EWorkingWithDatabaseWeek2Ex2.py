...
it might take as long as a few minutes to process all the data. The commit insists on completely writing all the data to disk every
time it is called.
The program can be speeded up greatly by moving the commit operation outside of the loop. In any database program, there is a
...
sqlstart = 'SELECT org, count FROM Counts ORDER BY count DESC LIMIT 10'

for row in cur.execute(sqlstart):
